...
volatile bool has_received_seq = false;
...
int ws_send_text(const client *client, const char *text) {
    ...
    const int sent = client_send(client, (const char*)frame, (int)(idx + len));
...
void send_heartbeat(const client *client) {
    ...
    if (__atomic_load_n(&has_received_seq, __ATOMIC_SEQ_CST)) {
